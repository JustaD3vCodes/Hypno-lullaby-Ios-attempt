...
class FlxHitbox extends FlxSpriteGroup
{
	...
	public function new(alt:Bool)
	{
		...
		{
			add(buttonLeft = createHint(0, 0, Std.int(FlxG.width / 4), Std.int(FlxG.height), 0xFF00FF));
			add(buttonDown = createHint(FlxG.width / 4, 0, Std.int(FlxG.width / 4), Std.int(FlxG.height), 0x00FFFF));
			add(buttonUp = createHint(FlxG.width / 2, 0, Std.int(FlxG.width / 4), Std.int(FlxG.height), 0x00FF00));
			add(buttonRight = createHint((FlxG.width / 2) + (FlxG.width / 4), 0, Std.int(FlxG.width), Std.int(FlxG.height), 0xFF0000));
		}
		scrollFactor.set();
	}
