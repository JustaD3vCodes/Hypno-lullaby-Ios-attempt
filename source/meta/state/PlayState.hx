...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (bronzongMechanic)
		{
			keysArray = [
				...
				copyKey(Init.gameControls.get('BACK')[0])
			];
		}
		else
		{
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inCutscene && generatedMusic)
		{
			...
			if (_virtualPad.buttonP.justPressed && startedCountdown
			...
			if (!deadstone)
			{
				...
				else
				{
					...
					if (pendulum != null && tranceActive)
					{
						...
						if (!tranceNotActiveYet)
						{
							...
							// /*
							if (controls.BACK || (strumLines.members[playerLane].autoplay && canHitPendulum && !alreadyHit))
							{
								if (canHitPendulum)
								{
									canHitPendulum = false;
				...
				if ((useFrostbiteMechanic
					...
					&& (strumLines.members[playerLane].autoplay
						&& coldness >= 0.5
						|| (controls.BACK  && !strumLines.members[playerLane].autoplay)))
				{
					useTyphlosion();
				}
	...
	function noteCalls()
	{
		...
		if (generatedMusic && startedCountdown)
		{
			for (strumline in strumLines)
			{
				...
				{
					if ((gameplayMode == PUSSY_MODE && daNote.noteData != 4) || (gameplayMode != PUSSY_MODE))
						{
